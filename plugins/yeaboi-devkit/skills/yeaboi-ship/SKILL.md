...
Ship the current feature branch. Arguments (optional): the user’s arguments — may include `auto-merge` to
...
**Read `.agents/repo-notes.md` before you start, if this repo has one.** It carries the facts this
...
2. **Commit.** Stage the relevant changes and commit with a lowercase imperative message (e.g. "add
   streaming output"), using attribution for the assistant that actually contributed.

   **Skip the pre-commit hook that re-runs the tests**, if this repo has one — `.agents/repo-notes.md`
...
4. **Independent verification (fresh context, no author bias) — IN THE BACKGROUND.** Use a fresh review session with the checklist in `../../agents/code-reviewer.md`.
   Use the assistant’s subagent facility when available; otherwise run an independent CLI session. Give it ONLY: (a) the output of **`git diff origin/main...HEAD`**, and
...
7. **Last mile, then push + PR.** `main` moves during a ship. Before pushing:
   ...
   - Body: a Summary section (what and why), a Test plan section (what was run), and accurate attribution for the assistant used (omit an AI footer if unknown).
   ...
   **Some repos rewrite the branch a minute after the push** — a version-bump commit pushed onto the
   PR branch by CI, for instance. Where that is true, `.agents/repo-notes.md` says so and says what
...
10. **Report** — output the PR URL and a one-line status, ending with what is still outstanding: the
    ...
    - **The rest of the set.** Run `make wt-siblings` from this worktree. It derives
      the full name, including nested names, from `.worktree.env` or the managed path.
