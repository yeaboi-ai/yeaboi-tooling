...
Worktree operations for parallel feature development. Arguments: the user’s arguments
...
- `new <name>` — run `make wt-new NAME=<name>`. Cuts `.worktrees/<name>` in **every** repo in
  ...
  a generated `<workspace>/.worktrees/<name>.code-workspace`, with a selected Claude or Codex session that has
...
- `rm <name>` — first check every worktree of that name is clean (`git -C <path> status --porcelain`,
  ...
  `<name>` is the path after `.worktrees/`, slashes included (`desktop/feature`, never just
  `feature`) — take it from `make wt-sets` output or `${PWD#*/.worktrees/}`, not from
...
Worktrees live under `<main checkout>/.worktrees/`, in every repo. If the current directory is
